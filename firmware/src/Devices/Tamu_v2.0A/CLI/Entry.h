...
static int SendMemoryExtra(uint16_t addr, uint8_t block, ServiceType service, uint8_t cid, uint8_t cli_srv_cid)
{
    ...
    if (service == ServiceType::Register) {
        ...
        switch (cid) {
            ...
            case 2: reg_cid = 0x12; break;  // Get Name -> Register 0x12
            case 3: reg_cid = 0x13; break;  // Set Name -> Register 0x13
...
static int CmdCreate(int argc, char **argv)
{
    ...
    {
        printf("Error: Only Register service supported for block creation.\n");
        return 1;
    }
    ...
    // Register service CID 0x10 (Create Dynamic) expects: BlockInfo + name
    // BlockInfo: Type=0x3FF (Dynamic), Instance=0 (auto-assign), Field=0xFF, Key=0xFF
    uint32_t block_info = ((0x3FF & 0x3FF) << 22) | ((0 & 0x3F) << 16) | (0xFF << 8) | 0xFF;

    uint8_t payload[MAX_PAYLOAD_SIZE];
    uint16_t plen = 0;
    memcpy(payload + plen, &block_info, 4); plen += 4;
    memcpy(payload + plen, name, name_len); plen += name_len;

    printf("Creating dynamic block (Type 0x%03X) named \"%s\" on device %d...\n",
           type, name, addr);

    PacketFrame req;
    PacketConstruct(&req, addr,
                     MakeService(ServiceType::Register, 0x10),  // Register service CID 0x10 (Create Dynamic)
...
static int CmdDevice(int argc, char **argv)
{
    ...
    else if (strcmp(cmd, "type") == 0 || strcmp(cmd, "sn") == 0 || strcmp(cmd, "version") == 0 || strcmp(cmd, "cap") == 0 || strcmp(cmd, "name") == 0 || strcmp(cmd, "uptime") == 0 || strcmp(cmd, "loop") == 0)
    {
        // Per docs these are Register System block type 0 fields, not Device service.
        // Use Register 01.01 Read with BlockInfo  Type0|Inst0|Field|Key
        uint32_t bi=0;
        if (strcmp(cmd, "type")==0) bi = (0u<<22) | (0u<<16) | (0u<<8) | 0u; // field0 key0
        else if (strcmp(cmd, "sn")==0) bi = (0u<<22) | (0u<<16) | (1u<<8) | 0xFFu;
        else if (strcmp(cmd, "version")==0) bi = (0u<<22) | (0u<<16) | (0u<<8) | 2u;
...
void StartCLI(void)
{
    ...
    const esp_console_cmd_t ping_cmd = {
        .command = "ping",
        .help    = "Ping a device (Device 00.01)",
        .hint    = "<addr>",
        .func    = &CmdPing,
        .argtable = nullptr,
    ...
    const esp_console_cmd_t identify_cmd = {
        .command = "identify",
        .help    = "Identify a device - makes it blink (Device 00.02)",
        .hint    = "<addr>",
        .func    = &CmdIdentify,
        .argtable = nullptr,
        .func_w_context = nullptr,
    ...
    ESP_ERROR_CHECK(esp_console_cmd_register(&identify_cmd));
