...
public:
    ...
    static void SetupBridge(BaseClass *Base, int32_t Index) { static_cast<BoardClass *>(Base)->Setup(Index); }
...
BoardClass::BoardClass(IDClass ID, FlagClass Flags) : BaseClass(&Table, ID, Flags) // Board always loads first, it takes ID 1
{
