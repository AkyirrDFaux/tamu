...
{
    Reference CurrentRef = Location;

    for (uint8_t Depth = 0; Depth < 4; Depth++)
    {
        // 1. Resolve the data location
        FindResult Result;
        
        if (CurrentRef.IsGlobal()) 
        {
            // For Global, we must find the external object
            BaseClass* ExternalObj = Objects.At(CurrentRef);
            if (!ExternalObj) return nullptr;
            
            // Recurse/Jump into the external object's values
            // We pass the same Type requirement down
            return ExternalObj->Values.Get(CurrentRef, Type);
        }
        else 
        {
            // For Local, search the current ByteArray
            Result = Find(CurrentRef);
        }

        // 2. Validate Find Results
        if (Result.Header < 0)
            return nullptr;

        // 3. If it's a direct type match, we are done
        if (Result.Type == Type)
            return Result.Value;

        // 4. Handle Redirection (Pointer to another Reference)
        if (Result.Type == Types::Reference)
        {
            Reference Redirect = Get<Reference>(CurrentRef).Value;
            if (!Redirect.IsValid()) return nullptr;
